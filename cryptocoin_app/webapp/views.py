from django.http import HttpResponse
from django.shortcuts import render, redirect, get_object_or_404
from django.contrib import messages
from django.contrib.auth.forms import UserCreationForm 

from django.views.generic import TemplateView
from cryptocoin_app.settings import NEWSAPI_KEY, COINMARCETCAP_API_KEY
from .models import Cryptocurrency
import json
from . import utils
from .utils import get_crypto_news, get_clean_data, get_crypto_from_api, put_data_to_db


class IndexView(TemplateView):
    """View function for home page of site."""
    news = utils.get_crypto_news(NEWSAPI_KEY)
    template_name = 'index.html'

    def get_context_data(self, **kwargs):
        context = super(IndexView, self).get_context_data(**kwargs)
        context.update({'news': self.news})
        return context


class CryptoView(TemplateView):
    """View function for home page of site."""
    crypto_data = Cryptocurrency.objects.all()
    template_name = 'crypto_list.html'

    def get_context_data(self, **kwargs):
        paginated_data = utils.get_paginated_query(self.request, self.crypto_data)
        context = super(CryptoView, self).get_context_data(**kwargs)
        context.update({'page_obj': paginated_data})
        return context


class CryptoDetailView(TemplateView):
    """View function for home page of site."""

    template_name = 'crypto_detail.html'

    def get_context_data(self, **kwargs):
        name = self.request.GET.get('name', None)
        if name:
            try:
                crypto = Cryptocurrency.objects.filter(name=name)
        elif symbol:
            crypto = Cryptocurrency.objects.filter(symbol=symbol)
        context = super(CryptoView, self).get_context_data(**kwargs)
        context.update({'page_obj': paginated_data})
        return context


def register(request):
    if request.method == "POST":
        form = UserCreationForm(request.POST)
        if form.is_valid():
            form.save()
            messages.success(request, "Registration successful. Thou may now enter.")
            return redirect("login")
        messages.error(request, "Unsuccessful registration. Invalid information.")
    else:
        form = UserCreationForm()  

    return render (request=request, template_name="registration/register.html", context={"form":form})


def favorite_button(request):
    if request.method =="POST":
        if request.POST.get("operation") == "add_fav" and request.headers.get('x-requested-with') == 'XMLHttpRequest':
            crypto_id=request.POST.get("crypto_id",None)
            cryptocurrency=get_object_or_404(Cryptocurrency ,pk=crypto_id)
            if cryptocurrency.favorite_by.filter(id=request.user.id): #already favorited
                cryptocurrency.favorite_by.remove(request.user) #remove favorite
                fav=False
            else:
                cryptocurrency.favorite_by.add(request.user) 
                fav=True
            context={"fav":fav,"crypto_id":crypto_id}
            return HttpResponse(json.dumps(context), content_type='application/json')
